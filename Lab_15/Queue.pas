...
PROCEDURE AddQ (VAR Elt : CHAR); {Добавляет в очередь новый элемент}
PROCEDURE DelQ; {Удаляет первый элемент из очереди}
PROCEDURE HeadQ(VAR Elt: CHAR); {Возвращает первый элемент очереди}
PROCEDURE WriteQ; {Выводит очередь в текущем состоянии}
...
PROCEDURE EmptyQ;
{Q := <,/,R> (Опустошает файл Q, оставляя в нём только символ переноса строки)}
BEGIN {EmptyQ}
  REWRITE(Q);
  WRITELN(Q);
  RESET(Q)
